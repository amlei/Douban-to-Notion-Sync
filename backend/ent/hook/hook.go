...
type CommunityMetaFunc func(context.Context, *ent.CommunityMetaMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CommunityMetaFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CommunityMetaMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CommunityMetaMutation", m)
}

// The FlomoMemoFunc type is an adapter to allow the use of ordinary
// function as FlomoMemo mutator.
type FlomoMemoFunc func(context.Context, *ent.FlomoMemoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f FlomoMemoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.FlomoMemoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.FlomoMemoMutation", m)
}
